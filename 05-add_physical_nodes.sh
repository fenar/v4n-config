...
maas admin machines create architecture=amd64 power_type=amt power_parameters_power_address=172.27.${ob_number}.${amtnum} power_parameters_power_pass=Password1+ mac_addresses=${mac} hostname=node${nodenum}ob${ob_number}
...
system_id=$(maas admin nodes read mac_address=$mac | grep system_id | cut -d'"' -f4)
echo $system_id
